...
///** 入队 - 有头结点 */
void enqueue4AdjList(LinkedQueue * linkedQueue, ElementType element){
//创建一个新结点
    QueueNode * node = (QueueNode *)malloc(sizeof(QueueNode));
    node->data = element;
...
///** 出队 */
void dequeue4AdjList(LinkedQueue * linkedQueue, ElementType *element){
    //如果首尾相连，证明队列为空
...
//
///** 入队 - 无头结点 */
//void OfferLinkedQueue(LinkedQueue * linkedQueue, ElementType element){
//    //创建一个新结点
//    QueueNode * node = (QueueNode *)malloc(sizeof(QueueNode));
//    node->data = element;
//    node->next = NULL;      //在队尾添加，所以新结点的next指向NULL
...
//
///** 出队 */
//void PollLinkedQueue(LinkedQueue * linkedQueue, ElementType *element){
